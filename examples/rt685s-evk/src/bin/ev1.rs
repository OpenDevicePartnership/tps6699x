...
async fn interrupt_task(
    mut int_in: Input<'static>,
    mut interrupt0: Interrupt<'static>,
    mut interrupt1: Interrupt<'static>,
) {
    embassy::task::interrupt_task(&mut int_in, [&mut interrupt0, &mut interrupt1]).await;
}

#[embassy_executor::task]
async fn pd_task(mut pd0: Tps6699x<'static>, mut pd1: Tps6699x<'static>) {
    let fw = include_bytes!("../../TPS66994_Host.bin");

    let mut controllers = [&mut pd0, &mut pd1];
...
async fn main(spawner: Spawner) {
    ...
    let int_in = Input::new(p.PIO0_25, Pull::Up, Inverter::Disabled);
    static BUS: OnceLock<Mutex<NoopRawMutex, I2cMaster<'static, Async>>> = OnceLock::new();
    let bus = BUS.get_or_init(|| {
        Mutex::new(I2cMaster::new_async(p.FLEXCOMM2, p.PIO0_15, p.PIO0_16, Irqs, Speed::Standard, p.DMA0_CH5).unwrap())
    });
    ...
    spawner.must_spawn(pd_task(pd0, pd1));
}
